...
export class StyleManager {
  ...
  constructor(private view: FoliateView, initialSettings: ViewSettings) {
  ...
  updateSettings(newSettings: Partial<ViewSettings>): void {
    const changed = Object.keys(newSettings).some(
      (key) =>
        this.currentSettings[key as keyof ViewSettings] !==
        newSettings[key as keyof ViewSettings]
    )
    if (changed) {
  ...
  updateLayout(dimensions: LayoutDimensions): void {
    ...
    this.view.renderer.setAttribute(
      'max-column-count',
      String(layout.maxColumnCount)
    )
    this.view.renderer.setAttribute(
      'max-inline-size',
      `${layout.maxInlineSize}px`
    )
    this.view.renderer.setAttribute(
      'max-block-size',
      `${layout.maxBlockSize}px`
    )
    this.view.renderer.setAttribute(
      'gap',
      `${this.currentSettings.gapPercent || 4}%`
    )
    // Apply flow mode
  ...
  private calculateLayout(dimensions: LayoutDimensions): {
    ...
    let computedMaxInlineSize =
      this.currentSettings.maxInlineSize ||
      getMaxInlineSize(this.currentSettings)
    let computedMaxColumnCount = this.currentSettings.maxColumnCount || 2
    const computedMaxBlockSize =
      this.currentSettings.maxBlockSize || Math.max(blockSize, 1440)
